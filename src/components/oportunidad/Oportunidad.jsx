...
export default function Oportunidad({ navigation }) {
    const localId = useSelector(state => state.authReducer.value.localId)
    const [principal, setPrincipal] = useState(true)
    const dat = useSelector(state => state.prospectoReducer.lista)
    const data = dat.filter(u => u.estado === "Oportunidad")
                ...
                {/* Filas */}
                {
                    datos?.map((item) => (
                        <View key={item.id} style={styles.tableRow}>
                            ...
                            <TouchableOpacity style={styles.detailButton} onPress={() => navigation.navigate('DetalleOportunidad', {
